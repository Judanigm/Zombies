...
@export var damage_radius: float = 180.0
...
const EXPLOSION_TEXTURE := preload("res://assets/Power ups/Granada/Explosión.png")
...
func _ready() -> void:
	...
	call_deferred("_apply_explosion_damage")


func _apply_explosion_damage() -> void:
	var strong_zombie_kill_count := 0
	...
	for zombie in get_tree().get_nodes_in_group("zombies"):
		...
		if zombie_node.global_position.distance_to(global_position) > damage_radius:
			continue

		var was_strong_zombie := _is_strong_zombie(zombie_node)
		var applied_damage := damage_amount
		if zombie_node.has_method("get_explosion_damage_amount"):
			applied_damage = zombie_node.get_explosion_damage_amount(damage_amount)

		if zombie_node.has_method("take_damage"):
			zombie_node.take_damage(applied_damage)
		elif zombie_node.has_method("die"):
			zombie_node.die()

		if was_strong_zombie and not zombie_node.is_in_group("zombies"):
			strong_zombie_kill_count += 1

	if strong_zombie_kill_count >= 3:
		var scene_root := get_tree().current_scene
		if scene_root != null and scene_root.has_method("register_grenade_strong_zombie_kills"):
			scene_root.register_grenade_strong_zombie_kills(strong_zombie_kill_count)
...
func _is_strong_zombie(zombie: Node) -> bool:
	if zombie == null:
		return false
	if zombie.has_method("is_strong_zombie"):
		return bool(zombie.call("is_strong_zombie"))
	return false
